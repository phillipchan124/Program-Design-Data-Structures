...
int main()
{
  ...
  const SortableStaticArray<int, 100> copy = a;
  ...
  cout << "\nThe stored values are: " << a.size() << endl;
  cout << "The values are:" << endl;
  a.sort();
  const SortableStaticArray<int, 100> copysort = a;
  for (int i = 0; i < copysort.capacity(); i++)
    if (copysort.containsKey(i))
      cout  << " " << i  << " " << copysort[i] << endl;
}