// Lab 15b, Depth First Search, DFS
// Programmer: Kwun Hang Chan
// Editor(s) used: Xcode
// Compliers(s) used: Apple LLVM version 6.1.0 (clang-602.0.53) (based on LLVM 3.6.0svn)#include <fstream>
#include <iostream>
#include <list>
#include <queue>
#include <string>
...
queue<int> doDepthFirstSearch(int iOriginVertex, vector<Vertex>& database)
{
  ...
  {
    int j = toDoList.top(); toDoList.pop();
    if (!database[j].isVisited)
    {
      database[j].isVisited = true;
      searchOrder.push(j);
      for ( it = database[j].adjacentVertices.rbegin();
           it != database[j].adjacentVertices.rend();
           it++)
        if(database[*it].isVisited == false) toDoList.push(*it);
    }
  }
  return searchOrder;
}
